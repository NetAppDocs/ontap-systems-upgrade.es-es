...
| ¿Estoy utilizando una versión de software diferente en los nodos original y nuevo? | Actualiza la versión de ONTAP en los nodos originales a la versión más alta soportada. Cuando sea posible, los nodos nuevos deben soportar la versión de ONTAP que está ejecutándose en los nodos originales. Si esto no es posible, link:https://docs.netapp.com/us-en/ontap/upgrade/prepare.html["Actualice la versión de ONTAP"^] en los nodos originales a la versión más alta soportada. No puedes tener una diferencia de versión mayor a cuatro entre los nodos originales y los nuevos. Por ejemplo, ONTAP 9.8 y 9.12.1 es compatible; sin embargo, ONTAP 9.8 y 9.13.1 no es compatible. link:https://docs.netapp.com/us-en/ontap/upgrade/concept_mixed_version_requirements.html["Obtenga más información sobre los clústeres mixtos de ONTAP"^]. 


| ¿Quiero realizar una actualización disruptiva? | link:upgrade/upgrade-decide-to-use-this-guide.html["Actualizar moviendo almacenamiento"]. Esto requiere un nuevo sistema con su propio almacenamiento y estantes para guardar los datos del sistema original. Puedes convertir desde un estante de controlador para ciertas actualizaciones. 
...
* Convierte el sistema original en un estante para unidades y traslada el almacenamiento:
+
** link:upgrade/upgrade_aff_a250_to_aff_a400_ndu_upgrade_workflow.html["Convierte el AFF A250 en un estante para unidades y mueve el almacenamiento al AFF A400"]
** link:upgrade/convert-to-drive-shelf.html["Convierte AFF A150, AFF A220 o FAS2820 en un estante para unidades y mueve el almacenamiento a un sistema de reemplazo compatible"]
...
| ¿Tengo un sistema modular? *Nota*: en un sistema modular, el controlador no tiene discos internos; todo el almacenamiento está en estantes conectados externamente. | link:upgrade-arl/index.html["Actualización mediante la reubicación de agregados"]. 
...
| ¿Tengo un sistema integrado y quiero actualizarlo cambiando solo el módulo controlador, conservando el chasis y los discos existentes? | link:upgrade-arl-auto-in-chassis/index.html["Actualiza usando ARL, manteniendo el mismo chasis, los discos y los datos almacenados en los discos"]. 
...
Este ejemplo te da posibles respuestas para una actualización de controlador FAS9500 a FAS90.

...
.. A medida que se realiza la reparación de alcanzabilidad, ONTAP intenta colocar los puertos en los dominios de difusión correctos. Sin embargo, si ONTAP no puede determinar la alcanzabilidad de un puerto y el puerto no corresponde a ninguno de los dominios de difusión existentes, ONTAP crea nuevos dominios de difusión para estos puertos. Según sea necesario, puedes eliminar los dominios de difusión recién creados si todos sus puertos miembros se convertirán en puertos miembros de los grupos de interfaces. Eliminar dominios de difusión:
+
...
. [[map_98_7]]A partir de ONTAP 9.8, ONTAP modifica automáticamente los puertos de origen de los LIFs si los puertos se mueven entre dominios de difusión durante el procedimiento de reparación de accesibilidad de puertos de red. Si el puerto de origen de un LIF se movió a otro nodo o no está asignado, ese LIF se presenta como un LIF desplazado. Restaura los puertos de origen de los LIFs desplazados cuyos puertos de origen ya no existen o se reubicaron en otro nodo.
+

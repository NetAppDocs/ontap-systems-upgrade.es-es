...
Los LUNs y volúmenes todavía están alojados físicamente en los nodos antiguos. Debido a que los LUNs son reportados solo en las interfaces de los nodos antiguos, el host solo muestra rutas sobre los nodos antiguos. Para ver esto, ejecuta los comandos  `sanlun lun show -p` y  `multipath -ll -d` en el host y revisa las salidas de los comandos.
...
.El futuro
link:upgrade-create-aggregate-move-volumes.html["Cree un agregado y mueva volúmenes a los nuevos nodos"]

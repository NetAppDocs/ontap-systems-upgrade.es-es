...
[role="lead"]
Debes verificar que los puertos físicos de node2 se asignen correctamente a los puertos físicos de node4. Esto permite que node4 se comunique con otros nodos del clúster y con la red después de la actualización.
...
Si los puertos físicos del nodo2 no se asignan directamente a los puertos físicos del nodo4, se debe utilizar la sección posterior <<restore_network_configuration_on_node4_9151,Restaurar la configuración de red en el nodo 4>> para reparar la conectividad de red.
...
== Paso 1: Verificar que el nodo4 está en quórum

Verifica que el nodo4 está en quórum. Luego, reanuda la operación de reemplazo del controlador.
...
== Paso 2: Restaurar la configuración de red en el nodo4
...
.El futuro
Tras verificar la instalación de node4, link:restore_key-manager_config_node4.html["restaurar la configuración de key-manager en node4"].

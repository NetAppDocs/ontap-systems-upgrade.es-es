...
+
Conecte las siguientes conexiones, utilizando las instrucciones de las _Instrucciones de instalación y configuración_ para la plataforma node4, el documento del estante de discos apropiado y la _administración de pares HA_.
...
+
Ingresa la frase de contraseña y la información de copia de seguridad que anotaste antes en el procedimiento. Consulta link:complete-preparation-for-upgrade.html#manage_storage_encryption_using_okm["Gestione el cifrado del almacenamiento con el gestor de claves incorporado"].
...
. [[auto_check_4_step11]]Cuando el nodo 4 se detenga en el menú de arranque, reasigne los discos del nodo 2 al nodo 4 ejecutando el siguiente comando en el nodo 4:
+
...
NOTE: En el ejemplo de salida de consola anterior, ONTAP te solicita el nombre de nodo asociado si el sistema utiliza discos de Advanced Disk Partitioning (ADP).

. [[auto_check_4_step27]]En el prompt LOADER, arranca el nodo:
+
`boot_ontap menu`
...
.El futuro
Después de instalar y arrancar el nodo4, link:verify_node4_installation.html["verifica la instalación de node4"].

...
[role="lead"]
Existen varios métodos de reubicación de agregados (ARL) para actualizar el hardware de los controladores. Este procedimiento ARL describe cómo actualizar un par HA de controladores en sistemas de almacenamiento AFF y FAS introducidos en ONTAP 9.15.1 o posterior con nuevos controladores, manteniendo los discos existentes y los datos almacenados en los discos.
...
Durante la actualización de un controlador, el controlador existente se sustituye por otro que ejecuta una plataforma más nueva o más potente. Este contenido proporciona los pasos para el procedimiento parcialmente automatizado, que utiliza comprobaciones automáticas de accesibilidad de puertos de red para simplificar aún más la experiencia de actualización del controlador.

.El futuro
Después de conocer este procedimiento de actualización ARL, link:decide_to_use_the_aggregate_relocation_guide.html["decidir si usar este procedimiento"].

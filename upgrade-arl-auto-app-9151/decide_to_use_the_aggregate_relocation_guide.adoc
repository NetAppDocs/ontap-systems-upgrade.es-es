...
[role="lead"]
Existen varios métodos de reubicación de agregados (ARL) para actualizar el hardware de los controladores. Este procedimiento de ARL describe cómo actualizar un par de controladores de HA en sistemas de almacenamiento AFF y FAS introducidos en ONTAP 9.15.1 o posterior con nuevos controladores, manteniendo los discos y los datos almacenados existentes. Este es un procedimiento complejo que solo deberían usar administradores de ONTAP con experiencia.

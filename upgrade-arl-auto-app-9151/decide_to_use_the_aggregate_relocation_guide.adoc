...
[role="lead"]
Existen varios métodos de reubicación de agregados (ARL) para actualizar el hardware de los controladores. Este procedimiento ARL explica cómo actualizar un par de controladores HA en sistemas de almacenamiento AFF y FAS introducidos en ONTAP 9.15.1 o versiones posteriores con nuevos controladores, manteniendo los discos existentes y los datos almacenados en ellos. Este es un procedimiento complejo que solo deberían usar administradores de ONTAP con experiencia.
...
.El futuro
Tras decidir utilizar este procedimiento ARL, link:required_tools_and_documentation.html["conoce las herramientas y la documentación necesarias"].

...
[role="lead"]
Debes verificar que los puertos físicos de node1 se asignen correctamente a los puertos físicos de node3. Esto permite que node3 se comunique con otros nodos del clúster y con la red después de la actualización.
...
La disposición física de los puertos puede variar, dependiendo del modelo de los nodos. Cuando el nuevo nodo arranca, ONTAP intenta determinar qué puertos deben alojar los LIF del clúster para entrar automáticamente en quórum.

Si los puertos físicos del nodo1 no se asignan directamente a los puertos físicos del nodo3, se debe utilizar la sección posterior <<restore_network_configuration_on_node3_9151,Restaurar la configuración de red en el nodo 3>> para reparar la conectividad de red.
...
En este punto del procedimiento, la operación se detiene ya que el nodo3 se une al quórum.



== Paso 1: Verifica que el nodo3 está en quórum

Verifica que el nodo3 está en quórum. Luego, reanuda la operación de sustitución del controlador
...
. La sustitución del controlador se detiene para intervenir con el siguiente mensaje:
+
....
...
== Paso 2: Restaurar la configuración de red en el nodo3
...
+
A medida que se realiza la reparación de alcanzabilidad, ONTAP intenta colocar los puertos en los dominios de difusión correctos. Sin embargo, si ONTAP no puede determinar la alcanzabilidad de un puerto y el puerto no pertenece a ninguno de los dominios de difusión existentes, ONTAP crea nuevos dominios de difusión para estos puertos.
...
.El futuro
Tras verificar la instalación de node3, link:restore_key-manager_configuration_node3.html["restaurar la configuración de key-manager en node3"].

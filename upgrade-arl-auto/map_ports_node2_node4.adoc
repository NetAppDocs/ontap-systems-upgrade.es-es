...
[role="lead"]
Debes verificar que los puertos físicos en el nodo2 se asignen correctamente a los puertos físicos en el nodo4. Esto permite que el nodo4 se comunique con otros nodos del clúster y con la red después de la actualización.
...
La configuración de los puertos puede variar, dependiendo del modelo de los nodos. Debes hacer que la configuración de puertos y LIF del nodo original sea compatible con la que planeas que sea la configuración del nuevo nodo. Esto es porque el nuevo nodo replica la misma configuración cuando arranca, lo que significa que cuando arrancas el nodo4, Data ONTAP intenta alojar LIFs en los mismos puertos que se usaron en el nodo2.

Por lo tanto, si los puertos físicos en node2 no se mapean directamente a los puertos físicos en node4, entonces se requieren cambios en la configuración del software para restaurar la conectividad del cluster, la gestión y la red después del arranque. Además, si los puertos del cluster en node2 no se mapean directamente a los puertos del cluster en node4, node4 podría no volver a unirse automáticamente al quórum cuando se reinicie hasta que se realice un cambio en la configuración del software para alojar los LIFs del cluster en los puertos físicos correctos.
...
+
Consulta link:other_references.html["Referencias"] para enlazar con _Network Management_ o _ONTAP 9 Command reference_ y mira _Configuring failover settings on a LIF_ para más información.

...
Si estás utilizando NetApp Aggregate Encryption (NAE) o NetApp Volume Encryption (NVE) para cifrar volúmenes en el sistema que estás actualizando, la configuración de cifrado debe sincronizarse con los nuevos nodos. Si no restauras key-manager, cuando reubicas los agregados de node1 de node2 a node3 usando ARL, los volúmenes cifrados se ponen fuera de línea.

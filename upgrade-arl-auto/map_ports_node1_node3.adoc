...
La configuración de los puertos puede variar en función del modelo de los nodos. Debes hacer que la configuración de puertos y LIF del nodo original sea compatible con el uso y la configuración previstos del nuevo nodo. Esto es porque el nuevo nodo reproduce la misma configuración cuando arranca, lo que significa que cuando arrancas el nodo3, ONTAP intenta alojar las LIF en los mismos puertos que se usaron en el nodo1.

Por lo tanto, si los puertos físicos de node1 no se asignan directamente a los puertos físicos de node3, entonces se requieren cambios en la configuración del software para restaurar la conectividad del clúster, la gestión y la red después del arranque. Además, si los puertos del clúster de node1 no se asignan directamente a los puertos del clúster de node3, es posible que node3 no vuelva a unirse automáticamente al quórum cuando se reinicie hasta que cambies la configuración del software para alojar los LIF del clúster en los puertos físicos correctos.
...
+
Consulta link:other_references.html["Referencias"] para enlazar con _Gestión de red_ o la _ONTAP 9 Command reference_ para más información.

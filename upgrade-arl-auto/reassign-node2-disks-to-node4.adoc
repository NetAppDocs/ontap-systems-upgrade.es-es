...
.Pasos
. [[reassign-node2-node4-step1]]Verifique que el nodo2 se haya detenido en el menú de arranque y reasigne los discos del nodo2 al nodo4:
+
...
----
====
. Si el sistema entra en un bucle de reinicio con el mensaje `no disks found` , esto se debe a que ha restablecido los puertos al modo de destino y, por lo tanto, no puede ver ningún disco.  Llevar a cabo<<auto_check_4_step3,Paso 3>> a través de<<auto_check_4_step8,Paso 8>> en el nodo 4 para resolver este problema.
...
NOTE: Es posible que se produzca un error en esta etapa.  Si se produce un error, repita los subpasos en<<auto_check_4_step8,Paso 8>> hasta que el sistema arranque normalmente.



. [[auto_check_4_step9]]Si está actualizando desde un sistema con discos externos a un sistema que admite discos internos y externos (sistemas AFF A800 , por ejemplo), configure el agregado del nodo 2 como el agregado raíz para confirmar que el nodo 4 arranca desde el agregado raíz del nodo 2.  Para configurar el agregado raíz, vaya al menú de arranque en el nodo 4 y seleccione la opción `5` para ingresar al modo de mantenimiento.
+

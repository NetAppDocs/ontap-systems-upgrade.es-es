...
summary: Después de intercambiar el módulo de controladora 2 AFF A900 correspondiente y el módulo NVRAM, puede que tenga que reiniciar el sistema. 
---
...
[role="lead"]
Tras intercambiar el módulo de controlador 2 AFF A900 o FAS9500 y NVRAM correspondientes, es posible que tenga que reiniciar el sistema. El término arranque desde red significa que se arranca desde una imagen ONTAP almacenada en un servidor remoto. Al prepararse para reiniciar el sistema, debe colocar una copia de la imagen de arranque ONTAP 9 en un servidor web al que pueda acceder el sistema.

No es posible comprobar la versión de ONTAP instalada en el soporte de arranque de un módulo de controlador AFF A900 o FAS9500 a menos que esté instalado en un chasis Y ENCENDIDO. La versión de ONTAP en los medios de arranque AFF A900 o FAS9500 debe ser la misma que la versión de ONTAP que se ejecuta en el sistema A700 o FAS9000 de AFF que está actualizando; las imágenes de arranque principal y de backup deben coincidir. Puede configurar las imágenes mediante el modo de reiniciar el sistema seguido de `wipeconfig` desde el menú de arranque. Si el módulo de la controladora se usaba anteriormente en otro clúster, el `wipeconfig` el comando borra toda la configuración residual del soporte de arranque.
...
. Espere a que el nodo 2 se ejecute ahora en el módulo del controlador AFF A900 o FAS9500 para arrancar y mostrar las opciones del menú de inicio, como se muestra en la siguiente salida:
+
[listing]
----
...
+
Para el nodo 2, la `partner-sysid` Debe ser el nodo 1 que actualice a un sistema AFF A900 o FAS9500.

...
Debes hacer que la configuración de puertos y LIF del nodo original sea compatible con la que planeas que sea la configuración del nuevo nodo. Esto se debe a que el nuevo nodo replica la misma configuración cuando arranca, lo que significa que cuando arrancas node3, ONTAP intenta alojar LIFs en los mismos puertos que se utilizaron en node1.

Por lo tanto, si los puertos físicos de node1 no se asignan directamente a los puertos físicos de node3, entonces se requieren cambios en la configuración del software para restaurar la conectividad del clúster, la gestión y la red después del arranque. Además, si los puertos del clúster de node1 no se asignan directamente a los puertos del clúster de node3, es posible que node3 no vuelva a unirse automáticamente al quórum cuando se reinicie hasta que se realice un cambio en la configuración del software para alojar los LIF del clúster en los puertos físicos correctos.
...
|===
+
Consulta en link:record_node1_information.html["Registre la información del nodo 1"] los pasos para obtener esta información.
...
----
+
Ve a link:other_references.html["Referencias"] para enlazar con _Gestión de red_ o la _ONTAP 9 Command reference_ para más información.

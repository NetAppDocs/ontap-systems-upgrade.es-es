...
[role="lead"]
Los puertos físicos del nodo2 deben asignarse correctamente a los puertos físicos del nodo4. Esto permite al nodo4 comunicarse con otros nodos del clúster y con la red después de la actualización.
...
Por lo tanto, si los puertos físicos de node2 no se asignan directamente a los puertos físicos de node4, entonces se requieren cambios en la configuración del software para restaurar la conectividad del clúster, la gestión y la red después del arranque. Además, si los puertos del clúster de node2 no se asignan directamente a los puertos del clúster de node4, es posible que node4 no vuelva a unirse automáticamente al quórum cuando se reinicie hasta que se realice un cambio en la configuración del software para alojar los LIF del clúster en los puertos físicos correctos.
...
+
Para más información, consulta link:other_references.html["Referencias"] para enlazar con _Network Management_ o _ONTAP 9 Command reference_, y ve a _Configuring failover settings on a LIF_.

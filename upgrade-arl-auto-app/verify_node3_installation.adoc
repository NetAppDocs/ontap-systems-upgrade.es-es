...
Si los puertos físicos del nodo1 no se asignan directamente a los puertos físicos del nodo3, se debe utilizar la sección posterior <<restore_network_configuration_on_node3,Restaurar la configuración de red en el nodo 3>> para reparar la conectividad de la red.
...
+
A medida que se realiza la reparación de alcanzabilidad, ONTAP intenta colocar los puertos en los dominios de difusión correctos. Sin embargo, si ONTAP no puede determinar la alcanzabilidad de un puerto y no pertenece a ninguno de los dominios de difusión existentes, ONTAP crea nuevos dominios de difusión para estos puertos.

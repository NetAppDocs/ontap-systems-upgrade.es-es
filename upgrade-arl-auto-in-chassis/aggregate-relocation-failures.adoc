...
+
Para obtener información detallada sobre las opciones  `storage aggregate relocation start`,  `override-vetoes` y  `override-destination-checks`, consulta link:other-references.html["Referencias"] para enlazar con la _ONTAP 9 Command reference_.
...
+
Cuando se sustituye el nodo2, aggr_node_1 se conecta con el nodo1 como nombre de nodo de origen en lugar del nodo2.
...
+
Para identificar los agregados que no se reubicaron correctamente, consulta la lista de agregados con el propietario de origen de node1 que obtuviste en la sección link:prepare-nodes-for-upgrade.html["Prepare los nodos para la actualización"] y compárala con la salida del comando anterior.

. Compara el resultado del paso 1 con el resultado que capturaste para node1 en la sección link:prepare-nodes-for-upgrade.html["Prepare los nodos para la actualización"] y fíjate en cualquier agregado que no se haya reubicado correctamente.

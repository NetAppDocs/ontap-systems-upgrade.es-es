...
== Paso 1: Verifica que el nodo1 está en quórum

Verifica que el nodo1 está en quórum. Luego, reanuda la operación de sustitución del controlador.
...
. Con el reemplazo del controlador en estado de pausa, procede a <<restore_network_configuration_on_node1,Restaurar la configuración de red en el nodo 1>>.




== Paso 2: Restaurar la configuración de red en el nodo1
...
.Acerca de esta tarea
Para más información sobre cómo crear y volver a crear VLANs, grupos de interfaces y dominios de difusión, consulta link:other-references.html["Referencias"] para enlazar con el contenido de _Network Management_.
...
.El futuro
Tras verificar la instalación del nodo1, link:restore-key-manager-config-upgraded-node1.html["restaura la configuración de key-manager en el nodo1 actualizado"].

...
[role="lead"]
Existen varios métodos de reubicación agregada (ARL) para actualizar el hardware de los controladores. Este procedimiento describe cómo actualizar los controladores de almacenamiento en un par de HA convirtiendo el sistema existente en el sistema de reemplazo, manteniendo el chasis del sistema existente, los discos y los datos almacenados en los discos.
...
NOTE: Los términos *nodo1* y *nodo2* se usan solo como referencia a los nombres de nodos en este documento. Cuando sigas el procedimiento, tienes que usar los nombres reales de tus nodos.



== Requisitos y limitaciones

Necesitas considerar factores importantes antes de empezar la actualización.


NOTE: Debes revisar toda la siguiente información importante.

* Este procedimiento es complejo y presupone que tienes habilidades avanzadas de administración de ONTAP. Debes leer y entender link:guidelines-for-upgrading-controllers-with-arl.html["Pautas para actualizar los controladores"] y link:overview-of-the-arl-upgrade.html["Secuencia de actualización de ARL"] antes de empezar la actualización.
...
* Si tienes un switch que no es compatible con la versión de ONTAP y el sistema de reemplazo al que estás actualizando, consulta link:other-references.html["Referencias"] para enlazar con el _Hardware Universe_.
...
* Si estás actualizando desde un sistema existente con tiempo de inactividad incluido en el link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matriz de sistemas compatibles"], puedes actualizar el hardware del controlador moviendo el almacenamiento o contactando al soporte técnico. Consulta link:other-references.html["Referencias"] para enlazar con _actualización moviendo volúmenes o almacenamiento_.




== Proceso automatizado de actualización de controladores
...
.El futuro
Después de conocer este procedimiento de actualización ARL, link:decide-to-use-the-aggregate-relocation-guide.html["decidir si usar este procedimiento"].

...
== Paso 1: Verifica que el nodo2 está en quórum

Verifica que el nodo2 está en quórum. Luego, reanuda la operación de sustitución del controlador.
...
. Con el reemplazo del controlador en estado de pausa, procede a <<restore_network_configuration_on_node2,Restaure la configuración de red en el nodo 2>>.




== Paso 2: restaurar la configuración de red en el nodo2
...
.El futuro
Tras verificar la instalación del nodo2, link:restore-key-manager-config-node2.html["restaurar la configuración de key-manager en node2"].

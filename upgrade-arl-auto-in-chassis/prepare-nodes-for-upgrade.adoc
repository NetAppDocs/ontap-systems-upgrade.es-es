...
Inicia el proceso de sustitución del controlador y ejecuta las comprobaciones previas. Si es necesario, determina el tipo de unidades de autocifrado que se están utilizando.
...
+
Ves una salida similar a la del siguiente ejemplo. La salida muestra la versión de ONTAP que se está ejecutando en tu clúster:
...
....
. Selecciona `y`. Ves el siguiente resultado:
+
....
...
| Comprobación de la versión de la imagen | Comprueba que los nodos que se están reemplazando ejecutan la misma versión de ONTAP. Si las versiones de imagen de ONTAP son diferentes, la tarea falla. Los nodos nuevos deben tener la misma versión de ONTAP 9.x instalada que la que está instalada en los nodos originales. Si los nuevos nodos tienen una versión diferente de ONTAP instalada, necesitas hacer netboot a los nuevos controladores después de instalarlos. Para instrucciones sobre cómo actualizar ONTAP, consulta link:other-references.html["Referencias"] para enlazar a _Upgrade ONTAP_. 
...
| Comprobación del estado del disco | Si alguno de los nodos que se están sustituyendo tiene discos que faltan o han fallado, la tarea falla. Si falta algún disco, consulta link:other-references.html["Referencias"] para enlazar a _Disk and aggregate management with the CLI_, _Logical storage management with the CLI_ y _HA pair management_ para configurar el almacenamiento del par de HA. 
...
== Paso 2: Corregir la propiedad del agregado si falla una comprobación previa de ARL
...
----
. Reinicia el proceso de sustitución del controlador:


`system controller replace start -nodes _node_names_`

.El futuro
Una vez preparados los nodos, link:complete-preparation-for-upgrade.html["completar la preparación para la actualización"].

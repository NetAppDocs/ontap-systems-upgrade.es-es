...
summary: Decide si vas a usar el procedimiento ARL que actualiza los modelos de controladoras en el mismo chasis, discos y los datos almacenados en los discos. 
---
...
[role="lead"]
Existen varios métodos de reubicación de agregados (ARL) para actualizar el hardware de los controladores. Este contenido explica cómo actualizar los controladores de almacenamiento en un par de HA convirtiendo el sistema existente en el sistema de reemplazo, manteniendo el chasis del sistema existente, los discos y los datos almacenados en los discos. Deberías usar este procedimiento complejo solo si eres un administrador de ONTAP con experiencia.
...
+
Para actualizar una configuración IP de MetroCluster, consulta link:other-references.html["Referencias"] para enlazar con el contenido de _MetroCluster Upgrade and Expansion_.
...
La siguiente tabla muestra la matriz de sistemas compatibles para realizar una actualización del controlador convirtiendo el sistema existente en el sistema de reemplazo, manteniendo el chasis del sistema existente, los discos y los datos almacenados en ellos.


CAUTION: Este procedimiento se aplica estrictamente a las siguientes configuraciones de actualización. No uses este procedimiento para realizar una actualización entre cualquier otra combinación de sistemas. Para todos los demás modelos de controladores, consulta link:other-references.html["Referencias"] para enlazar a _Use "system controller replace" commands to upgrade controller hardware running ONTAP 9.8 or later_ y _Using aggregate relocation to manually upgrade controller hardware running ONTAP 9.8 or later_.
...
| AFF A70 ^1^ | AFF A90 | 9.18.1 y posteriores 


| AFF A30 ^1^ | AFF A50 | 9.18.1 y posteriores 


| AFF A20 ^1^ | AFF A30 ^3^, AFF A50 | 9.18.1 y posteriores 


| AFF C30 ^1^ | AFF C60 | 9.18.1 y posteriores 


| FAS70 ^1^ | FAS90 | 9.18.1 y posteriores 
...
====
^3^ Existen ciertos requisitos de conexión de puertos para la actualización de un AFF A20 a un AFF A30 en una configuración de clúster de dos nodos sin switches:

* El AFF A20 es compatible con X60132A (4-port 10/25GbE CX7) y X60131A (2-port 10/25GbE CX6-LX) para la interconexión de clúster.
* El AFF A30 solo admite X60130A (2-port 100GbE CX6-DX) para la interconexión de clúster.
* Utiliza una tarjeta 10/25GbE temporal para conectar provisionalmente el AFF A20 al AFF A30. Una tarjeta 10/25GbE de 4 o 2 puertos es compatible con el clúster en el AFF A20. Se recomienda usar la misma tarjeta en el AFF A20 y en el AFF A30. Si no tienes una tarjeta nueva, puedes usar la tarjeta del AFF A20 como interconexión temporal del clúster.
* Cuando el nodo3 del AFF A30 se conecta, pone los puertos HA IC en 100GbE e intenta colocar los LIFs del clúster en los puertos de 10/25GbE. ONTAP detecta estos puertos como conectados al AFF A20. Si ONTAP no detecta estos puertos automáticamente, puedes modificar manualmente los puertos temporales del clúster. Luego, si es necesario, puedes mover los LIFs del clúster a los puertos temporales del clúster.
* Después de que actualices AFF A20 node1 y AFF A20 node2 se apague para prepararse para la actualización, necesitas quitar las conexiones de los puertos temporales del clúster de 10/25GbE y usar solo los puertos compartidos del clúster de 100GbE. Cuando AFF A30 node3 y node4 estén conectados por puertos de 100GbE, las conexiones se activan y puedes completar la actualización.

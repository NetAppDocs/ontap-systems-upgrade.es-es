...
[role="lead"]
El Node1 con los módulos de reemplazo está ahora listo para arrancar. Los módulos de reemplazo compatibles están listados en el link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matriz de sistemas compatibles"].
...
+
.. Respuesta `y` al mensaje de confirmación de plataforma mixta.
...
. [[A900_boot_node1]]Si tiene unidades NetApp Storage Encryption (NSE) instaladas, realice los siguientes pasos:
+
...
+
Ingresa la frase de contraseña y la información de copia de seguridad que anotaste antes en el procedimiento. Consulta link:complete-preparation-for-upgrade.html#manage-storage-encryption-using-okm["Gestione el cifrado del almacenamiento con el gestor de claves incorporado"].
...
. Cuando el nodo se detenga en el menú de arranque, reasigne los discos del nodo1 antiguo al nodo1 de reemplazo ejecutando el siguiente comando en el nodo1:
+
...
.El futuro
Tras arrancar el nodo1 con los módulos de sistema sustituidos, link:verify-node1-installation.html["verifica la instalación de node1"].

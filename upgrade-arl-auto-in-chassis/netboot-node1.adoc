...
No es posible comprobar la versión de ONTAP instalada en el medio de arranque del módulo controlador de sustitución a menos que esté instalado en un chasis y encendido. La versión de ONTAP en el medio de arranque del sistema de sustitución debe ser la misma que la versión de ONTAP que se está ejecutando en el sistema antiguo que estás actualizando y tanto la imagen de arranque principal como la de copia de seguridad en el medio de arranque deben coincidir. Para verificar la versión mínima de ONTAP admitida para tu actualización, consulta el link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matriz de sistemas compatibles"].
...
* Descarga los archivos de sistema necesarios para tu sistema y la versión correcta de ONTAP desde el _NetApp Support Site_. Consulta link:other-references.html["Referencias"] para acceder al _NetApp Support Site_.
...
.Pasos
. Consulta link:other-references.html["Referencias"] para enlazar al _NetApp Support Site_ y descargar los archivos usados para realizar el netboot del sistema.
...
+
.. En el siguiente indicador, ejecuta el comando `wipeconfig`. Luego selecciona Intro:
+
[listing]
----
Please choose one of the following:
...
.El futuro
Después de arrancar por red el nodo1, link:cable-node1-for-shared-cluster-HA-storage.html["cable node1 para cluster-HA y almacenamiento compartidos"].

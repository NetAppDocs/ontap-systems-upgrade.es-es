...
. [[step27]]ajuste el `bootarg.storageencryption.support` y.. `kmip.init.maxwait` variables para evitar un bucle de arranque después de cargar la configuración del nodo 1.
+
...
====
. [[step28]]nodo de arranque en el menú de arranque:
+
`boot_ontap menu`

+
Si no tiene configuración FC o UTA/UTA2, ejecute link:set_fc_or_uta_uta2_config_on_node3.html#auto9597_check_node3_step15["Comprobar y configurar los puertos UTA/UTA2 en el nodo 3, paso 15"] para que el nodo 3 pueda reconocer los discos del nodo 1.

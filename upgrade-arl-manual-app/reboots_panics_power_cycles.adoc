...
Algunos o todos los agregados han sido reubicados de node1 a node2, y HA sigue habilitada. Node2 se hace cargo del volumen raíz de node1 y de cualquier agregado no raíz que no haya sido reubicado.

.Acerca de esta tarea
La propiedad de los agregados que fueron reubicados tiene el mismo aspecto que la propiedad de los agregados no raíz que fueron tomados porque el propietario no ha cambiado. Cuando el nodo1 entra en `waiting for giveback state`, el nodo2 devuelve todos los agregados no raíz del nodo1.
...
.Acerca de esta tarea
El nodo1 se hace cargo de todos los agregados del nodo2, así como de cualquiera de sus propios agregados que hubiera reubicado en el nodo2. Cuando el nodo2 entra en el estado `Waiting for Giveback`, el nodo1 devuelve todos los agregados del nodo2.
...
. Sube el 2.
+
Se produce una interrupción del cliente para todos los agregados mientras node2 está arrancando.
...
El nodo 3 no toma el relevo tras una caída del nodo 2 porque HA ya está desactivada.
...
El nodo2 ha reubicado algunos o todos sus agregados en el nodo3, que sirve datos de los agregados que fueron reubicados. HA está deshabilitado.

.Acerca de esta tarea
Hay un corte de clientes para los agregados que no fueron reubicados.
...
Si node3 se bloquea mientras node2 está reubicando agregados en node3, node2 aborta la reubicación de cualquier agregado restante.
...
. Traiga el nodo 3.
+
Hay un corte de clientes para todos los agregados.

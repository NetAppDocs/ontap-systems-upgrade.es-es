...
Los LIFs remotos gestionan el tráfico a los LUNs SAN durante el procedimiento de actualización. Mover los LIFs SAN no es necesario para la salud del cluster o del servicio durante la actualización. Los LIFs SAN no se mueven a menos que necesiten ser mapeados a nuevos puertos. Necesitas verificar que los LIFs están sanos y ubicados en los puertos apropiados después de poner en línea node3.
...
.El futuro
Tras mover los LIF de datos NAS propiedad del nodo1 del nodo2 al nodo3 y verificar los LIF SAN en el nodo3, .

...
+
Ve a link:other_references.html["Referencias"] para enlazar con la _System Administration Reference_ para información sobre los SP y la _ONTAP 9 Command reference_ para información detallada sobre el comando  `system service- processor network modify`.
...
.El futuro
Después de confirmar que los nuevos controladores están configurados correctamente, link:set_up_storage_encryption_new_controller.html["configurar Storage Encryption en el nuevo módulo controlador"].

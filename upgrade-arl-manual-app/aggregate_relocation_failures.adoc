...
+
Para obtener información detallada sobre las opciones `storage aggregate relocation start`, `override-vetoes` y `override-destination-checks`, consulta link:other_references.html["Referencias"] para enlazar con la _ONTAP 9 Command reference_.
...
* Durante la Etapa 3, los agregados se reubican del nodo2 al nodo3. Algunos de los agregados que se reubican tienen node1 como su nodo de origen. Por ejemplo, un agregado de este tipo podría llamarse aggr_node_1. Si la reubicación de aggr_node_1 falla durante la Etapa 3 y no se puede forzar la reubicación, entonces el agregado se deja en node2.
* Después de la Etapa 4, cuando se sustituye el nodo2 por el nodo4. Cuando se sustituye el nodo2, aggr_node_1 entra en línea con el nodo4 como nodo de origen en lugar del nodo3.

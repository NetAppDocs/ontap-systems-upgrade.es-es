...
[role="lead"]
Debes asegurarte de que los puertos físicos de node1 se asignen correctamente a los puertos físicos de node3. Esto permite que node3 se comunique con otros nodos del clúster y con la red después de la actualización.
...
. [[man_map_1_step5]]A partir de ONTAP 9.8, ONTAP modifica automáticamente los puertos de origen de los LIF si los puertos se mueven entre dominios de difusión durante el procedimiento de reparación de accesibilidad de puertos de red. Si el puerto de origen de un LIF se movió a otro nodo o no está asignado, ese LIF se presenta como un LIF desplazado. Restaura los puertos de origen de los LIF desplazados cuyos puertos de origen ya no existen o se reubicaron en otro nodo.
+
...
.El futuro
Tras asignar los puertos del nodo1 al nodo3, link:move_nas_lifs_node1_from_node2_node3_verify_san_lifs_node3.html["mueve los LIFs de datos NAS propiedad de node1 de node2 a node3 y verifica los LIFs SAN en node3"].

...
[role="lead"]
Debes asegurarte de que los puertos físicos en node2 se asignen correctamente a los puertos físicos en node4. Esto permite que node4 se comunique con otros nodos del clúster y con la red después de la actualización.
...
.. A medida que se realiza la reparación de alcanzabilidad, ONTAP intenta colocar los puertos en los dominios de difusión correctos. Sin embargo, si ONTAP no puede determinar la alcanzabilidad de un puerto y el puerto no corresponde a ninguno de los dominios de difusión existentes, ONTAP intenta crear nuevos dominios de difusión para estos puertos. Según sea necesario, puedes eliminar los dominios de difusión recién creados si todos sus puertos miembros se convertirán en puertos miembros de los grupos de interfaces. Eliminar dominios de difusión:
+
...
. [[man_map_2_Step5]]A partir de ONTAP 9.8, ONTAP modifica automáticamente los puertos de origen de los LIF si los puertos se mueven entre dominios de difusión durante el procedimiento de reparación de accesibilidad de puertos de red. Si el puerto de origen de un LIF se ha movido a otro nodo o no está asignado, ese LIF se presenta como un LIF desplazado. Restaura los puertos de origen de los LIF desplazados cuyos puertos de origen ya no existen o se reubicaron en otro nodo.
+
...
.El futuro
Tras asignar los puertos del nodo2 al nodo4, link:move_nas_lifs_node2_from_node3_node4_verify_san_lifs_node4.html["mueve los LIFs de datos NAS propiedad de node2 de node3 a node4 y verifica los LIFs SAN en node4"].

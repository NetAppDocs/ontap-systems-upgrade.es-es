...
NOTE: Las actualizaciones de recuperación ante desastres de la máquina virtual de almacenamiento (SVM) no se interrumpen según los calendarios asignados.

No se producen cambios en la propiedad del agregado.

|===
.El futuro
Después de revisar el flujo de trabajo de actualización de ARL, link:guidelines_upgrade_with_arl.html["revisa las directrices para actualizar los controladores con ARL"].

...
.Pasos
. Verifique que el ID del sistema asociado esté configurado correctamente en el nodo 3:
+
...
. [[relocate-step5]]Reubicar los agregados que no son raíz:
+
...
Para más información sobre los comandos de reubicación de agregados de almacenamiento, ve a link:other_references.html["Referencias"] para enlazar con _Disk and aggregate management with the CLI_ y la _ONTAP 9 Command reference_.
...
.El futuro
Después de reubicar los agregados que no son raíz del nodo2 al nodo3, link:move_nas_lifs_node2_node3.html["mueve los LIF de datos NAS propiedad de node2 a node3"].

...
+
Si ni node1 ni node2 poseen agregados de los que son propietarios actuales (pero no propietarios de origen), el sistema devuelve un mensaje similar al siguiente ejemplo:
...
+
Para obtener información detallada sobre el  `network interface revert` comando, consulta link:other_references.html["Referencias"] para enlazar con la _ONTAP 9 Command reference_.
...
+
Las VLAN se configuran a través de puertos físicos. Si los puertos físicos cambian, entonces será necesario volver a crear las VLAN más adelante en el procedimiento.
...
+
Si ni iSCSI ni FC están configurados para la SVM, el comando muestra un mensaje similar al siguiente ejemplo:
...
+
Si se ha configurado iSCSI o FC para la SVM, el comando muestra un mensaje similar al del ejemplo siguiente:
...
+
Quizá quieras reutilizar los parámetros de red de los dispositivos de administración remota, en este caso los SP, del sistema original para los SP en los nuevos nodos. Para información detallada sobre el SP, consulta link:other_references.html["Referencias"] para enlazar a la _System Administration Reference_ y la _ONTAP 9 Command reference_.
...
+
Para obtener información detallada sobre AutoSupport, consulta link:other_references.html["Referencias"] para enlazar con la _Referencia de administración del sistema_ y la _Referencia de comandos de ONTAP 9_.
...
.El futuro
Tras preparar los nodos para la actualización, link:manage_authentication_okm.html["gestiona las claves de autenticación usando el Onboard Key Manager"].

...
+
La salida muestra `Done` para un agregado después de haber sido reubicado.
...
Consulta link:other_references.html["Referencias"] para enlazar con el contenido _Disk and aggregate management with the CLI_ y la _ONTAP 9 Command reference_ para más información sobre los comandos de reubicación de agregados de almacenamiento.
...
.El futuro
Después de reubicar los agregados que no son raíz de node1 a node2, link:move_nas_lifs_node1_node2.html["mueve los LIF de datos NAS propiedad de node1 a node2"].
